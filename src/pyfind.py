...
# 用迭代器节约内存
def finditer(
    match: str | FindFunc | Iterable[str], 
    root_dir: Path = Path("."), 
    ...
    Args:
        match (str | Callable[[str], bool] | Iterable[str]): 匹配条件，可以是字符串、符合签名的匹配函数或字符串集合。
    ...
    Returns:
        Deafult (Generator[Path, None, None]): 将要遍历目录的迭代器。
    """
    ...
    for root, dirs, files in os.walk(root_dir, followlinks=followlinks):
        ...
        for target in targets:
            ...
            if isinstance(match, str):
                ...
                elif isinstance(match, Iterable) and target in match:
                    matched = True
...
def find(
    ...
    Returns:
        Deafult (Path | None): 如果存在一个或多个匹配的文件，返回第一个文件路径，没有则返回None。
    """
